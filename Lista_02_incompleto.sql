...
CREATE OR REPLACE PACKAGE PKG_ALUNO IS
    PROCEDURE exclusao_de_aluno(id_aluno IN NUMBER);
END PKG_ALUNO;
/

CREATE OR REPLACE PACKAGE BODY PKG_ALUNO IS
    PROCEDURE exclusao_de_aluno(id_aluno IN NUMBER) IS
    BEGIN
    	DELETE FROM aluno
    	WHERE aluno.id_aluno = id_aluno;

		DBMS_OUTPUT.PUT_LINE('Aluno excluído');
	END exclusao_de_aluno;
END PKG_ALUNO;
/

BEGIN
    PKG_ALUNO.exclusao_de_aluno(1);
END;
...
BEGIN
    ...
    LOOP
		...
		IF TRUNC(v_data_nascimento) > TO_DATE ('2006-12-08', 'YYYY-MM-DD') THEN
...
DECLARE
    CURSOR cur_disciplina IS
        SELECT d.nome AS disciplina, COUNT(m.id_matricula) AS total_de_alunos
        FROM disciplina d
        JOIN matricula m ON d.id_disciplina = m.id_disciplina
    	GROUP BY d.nome;

    v_disciplina VARCHAR2(50);
	v_total_de_alunos NUMBER;
BEGIN
    OPEN cur_disciplina; 

    LOOP
        FETCH cur_disciplina INTO v_disciplina, v_total_de_alunos;
        EXIT WHEN cur_disciplina%NOTFOUND;
			IF v_total_de_alunos > 10 THEN
        		DBMS_OUTPUT.PUT_LINE('Disciplina: ' || v_disciplina);
			END IF;

    END LOOP;

    CLOSE cur_disciplina;
END;
/

DECLARE
    CURSOR cur_disciplina IS
        SELECT a.nome AS aluno, COUNT(d.id_disciplina) AS total_de_alunos
        FROM aluno a
        JOIN matricula m ON a.id_aluno = d.id_disciplina
    	GROUP BY d.nome;

    v_idade_total NUMBER := 0;
	v_qtd_alunos NUMBER := 50;
	v_idade_media NUMBER;
BEGIN
    OPEN cur_disciplina; 
    ...
    LOOP
		...
		v_idade_total := a.data_nascimento.NETXVAL;
		v_idade_media := v_idade_total / v_qtd_alunos;

        DBMS_OUTPUT.PUT_LINE('Média de idade dos matriculados: ' || v_idade_media);
    END LOOP;
...
CREATE OR REPLACE PROCEDURE disciplina_total_alunos(
    p_id_disciplina IN NUMBER,
    p_total_alunos OUT NUMBER
)
IS
BEGIN 
	SELECT COUNT(*) INTO p_total_alunos
	FROM matricula
	WHERE id_disciplina = p_id_disciplina;

	DBMS_OUTPUT_PUT_LINE('total de aluno' || p_total_alunos);
END disciplina_total_alunos;


----------------------------------------------------------------------------------------------------
CREATE OR REPLACE PACKAGE PKG_PROFESSOR IS
DECLARE    
    CURSOR cur_professores_disciplinas IS
    	SELECT p.nome AS professor, d.nome AS disciplinas
    	FROM professor p
	...
	v_disciplina VARCHAR2(50);
BEGIN
    OPEN cur_professores_disciplinas;

	LOOP
        FETCH cur_professores_disciplinas INTO v_professor, v_disciplina;
		EXIT WHEN cur_professores_disciplins%NOTFOUND;

		IF v_disciplina > 1 THEN
			DBMS_OUTPUT.PUT_LINE('Professor' || v_professor);
		END IF;

	END LOOP;
	CLOSE cur_professores_disciplinas;
	END;
END PKG_ALUNO;
/


CREATE OR REPLACE FUNCTION total_professor(p_id_professor NUMBER)
RETURN NUMBER
IS
	v_total NUMBER;
BEGIN
...
CREATE OR REPLACE FUNCTION ministra_na_disciplina (p_id_disciplina NUMBER)
RETURN NUMBER
IS
